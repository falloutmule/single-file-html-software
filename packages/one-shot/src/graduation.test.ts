...
describe("graduation migration records", () => {
  ...
  it("imports transactionally and materializes to an ignored workspace location", async () => {
    ...
    await expect(readFile(join(destination, "one-shot", "SFHS-PIN.json"), "utf8").then((text) => JSON.parse(text))).resolves.toMatchObject({ node: ">=22.18.0", pnpm: "11.9.0" });
    ...
    const workspace = join(root, "workspace"); await mkdir(join(workspace, "examples"), { recursive: true }); await mkdir(join(workspace, "adapters", "pixi-v8", "node_modules", "pixi.js"), { recursive: true }); await mkdir(join(workspace, "packages", "pixi-runtime"), { recursive: true }); await writeFile(join(workspace, "adapters", "pixi-v8", "package.json"), JSON.stringify({ name: "@sfhs/adapter-pixi-v8", dependencies: { "pixi.js": "8.19.0" } })); await writeFile(join(workspace, "packages", "pixi-runtime", "package.json"), JSON.stringify({ name: "@sfhs/pixi-runtime" })); await writeFile(join(workspace, "adapters", "pixi-v8", "node_modules", "pixi.js", "package.json"), JSON.stringify({ name: "pixi.js" })); const originalPackage = await readFile(join(destination, "package.json"), "utf8"); const materialized = await materializeGraduationProject({ sourceRoot: destination, workspaceRoot: workspace, projectId: "test-product", revision: "a".repeat(40) });
    expect(materialized.valid, JSON.stringify(materialized.findings)).toBe(true); expect(materialized.value?.path).toContain(".sfhs-grad-test-product-"); expect((await stat(join(materialized.value!.path, "node_modules", "@sfhs", "adapter-pixi-v8"))).isDirectory()).toBe(true); expect((await stat(join(materialized.value!.path, "node_modules", "pixi.js"))).isDirectory()).toBe(true); expect(await readFile(join(destination, "package.json"), "utf8")).toBe(originalPackage); const overlay = JSON.parse(await readFile(join(materialized.value!.path, "MATERIALIZATION.json"), "utf8")) as { workspaceLinkIdentities: readonly { name: string }[] }; expect(overlay.workspaceLinkIdentities.map((entry) => entry.name)).toEqual(["@sfhs/adapter-pixi-v8", "@sfhs/pixi-runtime"]); expect((await auditGraduationProject(materialized.value!.path)).valid).toBe(true); const pinPath = join(materialized.value!.path, "one-shot", "SFHS-PIN.json"); const pin = JSON.parse(await readFile(pinPath, "utf8")) as Record<string, unknown>; await writeFile(pinPath, JSON.stringify({ ...pin, sfhsCommit: "b".repeat(40) })); expect((await auditGraduationProject(materialized.value!.path)).findings).toEqual(expect.arrayContaining([expect.objectContaining({ code: "SFHS_GRAD_MATERIALIZATION_STALE" })]));
  });
  it("rejects unsafe or duplicate workspace overlay names without leaving a materialization", async () => {
    const root = await temporaryRoot(); const source = join(root, "source"); const workspace = join(root, "workspace"); await mkdir(join(source, "one-shot"), { recursive: true }); await mkdir(join(workspace, "examples"), { recursive: true }); await writeFile(join(source, "package.json"), "{}\n"); await writeFile(join(source, "sfhs.project.json"), "{}\n"); await writeFile(join(source, "one-shot", "SFHS-PIN.json"), JSON.stringify({ workspacePackages: ["@sfhs/pixi-runtime", "@sfhs/pixi-runtime"] }));
    const duplicate = await materializeGraduationProject({ sourceRoot: source, workspaceRoot: workspace, projectId: "duplicate", revision: "a".repeat(40) }); expect(duplicate.valid).toBe(false); expect(duplicate.findings[0]?.code).toBe("SFHS_GRAD_MATERIALIZATION_INVALID"); expect(await stat(join(workspace, "examples", ".sfhs-grad-duplicate-")).then(() => true).catch(() => false)).toBe(false);
    await writeFile(join(source, "one-shot", "SFHS-PIN.json"), JSON.stringify({ workspacePackages: ["@sfhs/../escape"] })); const traversal = await materializeGraduationProject({ sourceRoot: source, workspaceRoot: workspace, projectId: "traversal", revision: "a".repeat(40) }); expect(traversal.valid).toBe(false); expect(traversal.findings[0]?.code).toBe("SFHS_GRAD_MATERIALIZATION_INVALID");
  });
