...
export async function importGraduationProject(options: { readonly sourceRoot: string; readonly destination: string; readonly plan: MigrationPlan; readonly revision: string }): Promise<GraduationResult<{ readonly state: GraduationState }>> {
  ...
  try {
    ...
    await atomicDirectory(destination, async (staging) => {
      ...
      await writeChatJson(join(recordRoot, "SFHS-PIN.json"), { schema: "sfhs.graduation-sfhs-pin@1", sfhsRepository: "falloutmule/single-file-html-software", sfhsCommit: options.revision, node: ">=22.18.0", pnpm: "11.9.0", adapter: "pixi-v8", sourceProductIdentity: sourceHash, workspacePackages: ["@sfhs/adapter-pixi-v8", "@sfhs/pixi-runtime"] });
