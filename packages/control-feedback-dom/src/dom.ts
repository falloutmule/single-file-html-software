...
export function mountDomControlUnchecked(options: MountDomControlOptions): DomControlController {
  ...
  listen(elements.interactive, "pointerdown", (rawEvent) => {
    ...
    dispatch({ kind: "focus-set", focused: true, focusVisible: false, atMs: now() });
  ...
  listen(elements.interactive, "focus", () => dispatch({ kind: "focus-set", focused: true, focusVisible: !pointerFocusInProgress && elements.interactive.matches(":focus-visible"), atMs: now() }));
