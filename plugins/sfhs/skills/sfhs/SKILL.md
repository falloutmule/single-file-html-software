...
2. For a complete bounded game or interactive application, read `one-shot/START-HERE.md` first. For a Chat-authored build, use the opt-in Protocol v2 preflight and run-state before source work; it selects environment mode and a packet but does not replace the CLI. For durable migration of a completed Chat package or legacy intake, read `one-shot/instructions/graduation-protocol-v1.md` and begin with `sfhs one-shot graduate inspect`; graduation never performs remote actions or invents canonical evidence.
