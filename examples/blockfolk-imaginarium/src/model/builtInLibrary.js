...
import { BLOCKFOLK_VALLEY_ASSET } from './worldModel.js';
...
export const BUILT_IN_BACKGROUNDS = Object.freeze([BLOCKFOLK_VALLEY_ASSET]);
export const BUILT_IN_STICKERS = BLOCKFOLK_STICKERS;
export const BUILT_IN_ASSETS = Object.freeze([BLOCKFOLK_VALLEY_ASSET, ...BLOCKFOLK_STICKERS]);
...
export function validateBuiltInLibrary() {
  ...
  if (BUILT_IN_BACKGROUNDS.length !== 1 || BUILT_IN_STICKERS.length !== 30 || BUILT_IN_ASSETS.length !== 31 || !BUILT_IN_BACKGROUNDS[0].production || BUILT_IN_BACKGROUNDS[0].debug) throw new Error('BlockFolk must contain one production world and exactly 30 accepted stickers.');
  return { backgrounds: 1, stickers: BUILT_IN_STICKERS.length, categories: BUILT_IN_CATEGORIES.length };
}
