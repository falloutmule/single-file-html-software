...
export const BUILT_IN_ASSETS = Object.freeze([BLOCKFOLK_VALLEY_ASSET]);
export const CREATIVE_PROMPTS = Object.freeze([]);

export function findBuiltInAsset(assetId) { return assetId === BLOCKFOLK_VALLEY_ASSET.id ? BLOCKFOLK_VALLEY_ASSET : null; }
...
export function validateBuiltInLibrary() {
  ...
  if (BUILT_IN_BACKGROUNDS.length !== 1 || BUILT_IN_STICKERS.length || BUILT_IN_ASSETS.length !== 1 || !BUILT_IN_BACKGROUNDS[0].production || BUILT_IN_BACKGROUNDS[0].debug) throw new Error('BlockFolk must contain one production world and no built-in stickers.');
