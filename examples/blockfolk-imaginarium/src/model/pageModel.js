...
export const PAGE_SCHEMA = 'blockfolk-imaginarium.page@1';
...
export function createPicture({ id = createStableId('blockfolk-picture'), title = 'My BlockFolk Picture', backgroundAssetId = null, now = new Date().toISOString() } = {}) {
  return {
    schema: PAGE_SCHEMA,
    id,
    title,
...
export function validatePicture(value) {
  ...
  if (value.page?.width !== PAGE_WIDTH || value.page?.height !== PAGE_HEIGHT || (value.page?.backgroundAssetId !== null && typeof value.page?.backgroundAssetId !== 'string')) throw new Error('Picture page settings are invalid.');
...
export function createSticker(assetId, { x = PAGE_WIDTH / 2, y = PAGE_HEIGHT / 2, scale = 0.78, layerId = createStableId('blockfolk-sticker') } = {}) {
...
export function duplicateSticker(sticker, layerId = createStableId('blockfolk-sticker')) {
...
export function duplicatePicture(picture, title, now = new Date().toISOString()) {
  ...
  copy.id = createStableId('blockfolk-picture');
  copy.title = title || `${picture.title} Copy`;
  copy.createdAt = now;
  copy.updatedAt = now;
  copy.stickers = copy.stickers.map((sticker) => ({ ...sticker, layerId: createStableId('blockfolk-sticker') }));
  return copy;
}
