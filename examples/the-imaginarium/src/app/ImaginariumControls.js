...
export function createImaginariumControlPreset({ family = 'tool', palette = PALETTES.cream, shape = 'round-rect', semantic = { kind: 'momentary' }, value = 'control' } = {}) {
  ...
  return Object.freeze({
    ...
    id: `imaginarium-${safeToken(family)}-${safeToken(semantic.kind)}-${safeToken(value)}`,
    title: `Imaginarium ${family} control`,
...
function inferControlOptions(button) {
  ...
  if (isCategory) {
    ...
    semantic = { kind: 'choice', groupId: 'imaginarium-sticker-category', value };
    selected = button.getAttribute('aria-selected') === 'true';
  } else if (isBackground) {
    value = button.dataset.backgroundId;
    semantic = { kind: 'choice', groupId: 'imaginarium-background', value };
    selected = button.getAttribute('aria-pressed') === 'true';
  } else if (isAutosave) {
    value = button.dataset.autosaveMode;
    semantic = { kind: 'choice', groupId: 'imaginarium-autosave-mode', value };
    selected = button.getAttribute('aria-pressed') === 'true';
  } else if (isPuzzleDifficulty) {
    value = button.dataset.puzzleDifficulty;
    semantic = { kind: 'choice', groupId: 'imaginarium-puzzle-difficulty', value };
    selected = button.getAttribute('aria-pressed') === 'true';
...
export class ImaginariumControls {
  ...
  upgradeButton(button, overrides = {}) {
    ...
    const preset = createImaginariumControlPreset(options);
    const mount = document.createElement('span');
    let record;
    const controller = mountDomControl({
      container: mount,
      controlId: `imaginarium-${safeToken(options.value)}-${this.nextControlId++}`,
      preset,
      label,
